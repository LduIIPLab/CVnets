...
# -----------assist method, print dataset-not-supporting message &  supported dataset list
...
# ------assist method, add general dataset arguments
def general_dataset_args(parser: argparse.ArgumentParser):
    ...
    group.add_argument(
        "--dataset.train-index-file",
        type=str,
        default="",
        help="Index file location of train dataset",
    )
    group.add_argument(
        "--dataset.train-index-offset",
        type=int,
        default=0,
        help="Index offset of train dataset label",
    )
    group.add_argument(
        "--dataset.root-val",
        type=str,
    ...
    group.add_argument(
        "--dataset.val-index-file",
        type=str,
        default="",
        help="Index file location of valid dataset",
    )
    group.add_argument(
        "--dataset.val-index-offset",
        type=int,
        default=0,
        help="Index offset of valid dataset label",
    )
    group.add_argument(
        "--dataset.root-test",
        type=str,
    ...
    group.add_argument(
        "--dataset.test-index-file",
        type=str,
        default="",
        help="Index file location of test dataset",
    )
    group.add_argument(
        "--dataset.test-index-offset",
        type=int,
        default=0,
        help="Index offset of test dataset label",
    )
    group.add_argument(
...
for task in SUPPORTED_TASKS:
    ...
    for log_file in os.listdir(task_path):
        path = os.path.join(task_path, log_file)
        if (
            not log_file.startswith("_")
            and not log_file.startswith(".")
            and (log_file.endswith(".py") or os.path.isdir(path))
        ):
            dataset_name = log_file[: log_file.find(".py")] if log_file.endswith(".py") else log_file
